...
class WaypointApply(object):
    def __init__(self, trans_vel, ang_vel, goal_pose_tolerance, goal_angle_tolerance, region_marker_id, \
        region_pose_x, region_pose_y, marker_goal_pose_x, marker_goal_pose_y):
        self._trans_vel = trans_vel
        self._ang_vel = ang_vel
        self._goal_pose_tolerance = goal_pose_tolerance
        self._goal_angle_tolerance = goal_angle_tolerance
        self._cur_robot_pose_sub = rospy.Subscriber('/movo/odometry/local_filtered', Odometry, self.cur_robot_pose_cb)
        self._marker_detect_sub = rospy.Subscriber('/aruco_single/marker', Marker, self.marker_detect_cb)
        self._base_vel_pub = rospy.Publisher('/movo/cmd_vel', Twist, queue_size=10)
        ...
        self._region_marker_id = region_marker_id
        self._goal_pose_x = region_pose_x
        self._goal_pose_y = region_pose_y
        self._marker_goal_pose_x = marker_goal_pose_x
        self._marker_goal_pose_y = marker_goal_pose_y

        self._cur_robot_pose_check = False
        self._reach_goal_check = False
        self._marker_detect_check = False
    ...
    def marker_detect_cb(self, data):
        if self._reach_goal_check:
            self._reach_goal_check = False
            if not self._marker_detect_check:
                if data.id == self._region_marker_id:
                    self._goal_pose_x = data.pose.position.x+self._marker_goal_pose_x
                    self._goal_pose_y = data.pose.position.y+self._marker_goal_pose_y
                    self.waypoint_execute()
                    self._marker_detect_check = True

    def waypoint_execute(self):
        if self._cur_robot_pose_check:
            rospy.loginfo("Goal pose is (%.2f, %.2f)", self._goal_pose_x, self._goal_pose_y)
            diff_x = self._goal_pose_x-self._cur_robot_pose_x
            diff_y = self._goal_pose_y-self._cur_robot_pose_y
    ...
    def check_marker_detect(self):
        return self._marker_detect_check